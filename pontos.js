...
const pontosBox = document.querySelector(".pontos-box") || document.getElementById("listaPontos");
...
const btnNovoPonto = document.getElementById("btnNovoPonto") || document.querySelector(".btn-novo-ponto");
...
function calcularStatusInfo(ponto) {
  ...
  return {
    texto: "Inativo",
    detalhe: statusEhInativo(status) ? `Inativo desde ${horario}` : "Inativo desde sem histórico",
    ativo: false,
    classe: "inativo"
  };
}
...
async function criarNovoPonto() {
  ...
  try {
    if (!supabaseClient) {
      throw new Error("Supabase não carregou. Confira o script CDN no HTML.");
    }
...
async function buscarStatusPontosRemoto() {
  // A tabela statuspontos não existe em alguns ambientes. Manter vazio evita
  // erro 404 no console e não bloqueia o carregamento dos pontos.
...
async function buscarPontosRemoto() {
  ...
  for (const tentarBuscar of tentativas) {
    const { data, error } = await tentarBuscar();
    ...
    <div class="playlist-item" draggable="true" data-index="${index}" data-id="${item.id}">
      <div class="playlist-item-linha">
        ...
        <div class="playlist-item-acoes-laterais">
          ...
          <button class="playlist-acao btn-excluir-item" type="button" data-id="${item.id}" title="Excluir">&times;</button>
        </div>
      </div>
    </div>
  `;
...
async function salvarEdicaoPonto() {
  ...
  if (!nome || !cidade || !endereco) {
    setStatus("Preencha nome, cidade e endereço", "erro");
    return;
  }
  ...
  try {
    ...
    if (error) {
      ...
      setStatus("Erro ao salvar informações", "erro");
      return;
    }
...
async function copiarPlaylistInteira() {
  ...
  const confirmar = window.confirm("Copiar toda a playlist dessa pasta para o ponto atual? A playlist atual será substituída.");
  if (!confirmar) return;
    ...
    if (pontosBox) {
      pontosBox.innerHTML = `
        <div class="playlist-vazia">
          Erro ao carregar pontos: ${escapeHtml(error?.message || "falha no banco")}
        </div>
      `;
    }
...
document.addEventListener("click", (event) => {
  const botaoNovo = event.target.closest("#btnNovoPonto, .btn-novo-ponto, [data-acao='novo-ponto']");
  if (!botaoNovo) return;
